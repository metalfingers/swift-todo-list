...
//  todo list
//
//  Created by Richard Lewis on 7/16/15.
...
class StrikeThroughText: UILabel {
    ...
    func resizeStrikeThrough() {
        if let temp = text?.sizeWithAttributes([NSFontAttributeName:font]){
            let textSize = text!.sizeWithAttributes([NSFontAttributeName:font])
            strikeThroughLayer.frame = CGRect(x: 0.0, y: bounds.size.height/2, width: textSize.width, height: kStrikeOutThickness)
